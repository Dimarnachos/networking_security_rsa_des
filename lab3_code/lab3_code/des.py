...
class des():
    ...
    def run_cbc(self, key, text, action=ENCRYPT, padding=False, IV='ASASASAS'):
        ...
        for block in text_blocks:  # Loop over all the blocks of data
            ...
            if action == DECRYPT:
                result += self.xor(self.permut(d + g, PI_1),
                                   previous_block)  # Do the last permut and append the result to result
    ...
    def substitute(self, d_e):  # Substitute bytes using SBOX
        ...
        # ##################################your code goes here###################################
        # for each 6 bit subblock you need to apply the corresponding s box (using the compute_s_box function) and
        # save the result in result value
        # result is a list of integer values 1 or 0
        result = list()
        for i in range(len(subblocks)):
            block = subblocks[i]
            tempbin = self.compute_s_box(block, i)
    ...
    def permut(self, block, table):  # Permut the given block using the given table (so generic method)
        # ##################################your code goes here###################################
        # permute the block using the table! the table will be either PI or CP_1 or CP_2 (found in the beginning of
        # this file!)
        # block is a bit array in integers [1,0,1,...], the output has the same format as block but with a permutation
        # of the members
    ...
    def compute_s_box(self, block, round):
        ...
        row = (2 * block[0]) + (block[5])
        column = (8 * block[1]) + (4 * block[2]) + (2 * block[3]) + (block[4])
        new_box = S_BOX[round % 8]
        value = new_box[row][column]
        bin = binvalue(value, 4)
